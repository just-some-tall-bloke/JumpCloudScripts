...
# --- Configuration ---
# Parse arguments
parser = argparse.ArgumentParser(description="JumpCloud Duplicate System Remover")
parser.add_argument("--delete", action="store_true", help="Enable actual deletion. If not provided, script runs in dry-run mode.")
...
def process_duplicates(duplicate_groups: Dict[str, List[Dict[str, Any]]]) -> None:
    ...
    if DRY_RUN:
        ...
        logging.info(color_text("[INFO] To run for real, use the --delete flag: python3 duplicate-remover.py --delete", COLOR_INFO))
    else:
