...
"""
JumpCloud High Uptime Grouping Script
- Removes Mac computers that have rebooted (uptime <= 14 days)
- Removes non-Mac systems from the group
- Removes systems that haven't contacted JumpCloud in the last 7 days
"""
...
def categorize_systems_by_uptime(systems, uptime_threshold, contact_window_days=7):
    ...
    print(color_text(f"\nFound {len(mac_systems)} Mac systems", COLOR_INFO))
    print(
        color_text(
            f"Excluded: {len(excluded_not_mac)} non-Mac systems",
            COLOR_INFO
        )
    )
...
def identify_systems_to_remove_from_group(
    ...
    1. Non-Mac systems
    2. Systems with low uptime (rebooted)
    3. Stale systems (not contacted recently)
    """
    ...
    return low_uptime_in_group, stale_in_group, non_mac_in_group
...
def main():
    ...
    suggested_group_name = (
        f"Macs with uptime > {UPTIME_THRESHOLD_DAYS} days"
    )

    print(color_text("\n" + "=" * 60, COLOR_INFO))
    print(color_text(f"Target: Mac systems with high uptime", COLOR_INFO))
    ...
    low_uptime_in_group, stale_in_group, non_mac_in_group = (
